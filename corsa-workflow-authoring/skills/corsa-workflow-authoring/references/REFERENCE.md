...
Exhaustive reference for building workflow definitions in the Corsa no-code workflow engine. The
...
`missingAssociationBehavior?` (`FAIL|SKIP`). Per-actionType specifics:
...
Distinct from ACTION `SEND_EMAIL` (which sends **client-facing** templated email);
NOTIFY EMAIL is an **internal analyst** notification.
